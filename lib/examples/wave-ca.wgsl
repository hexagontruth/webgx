...
fn sampleCell(h: vec3i) -> vec2f {
  var p = fromHex(h);
  var offset = (p.x * u32(pu.bufferDim) + p.y) * 2;
  return vec2f(
...
fn fragmentMain(data: VertexData) -> @location(0) vec4f {
  ...
  c = hsv2rgb3(c);
  c *= htWhite;
  // c = max(c, vec3f(qw1(amax3(hex) - 1, 1/pu.gridRadius/16, 1/pu.gridRadius/2)));
  return vec4f(c, 1);
}

@fragment
fn fragmentTest(data: VertexData) -> @location(0) vec4f {
  var p = vec2u(round((data.cv * gu.cover * 0.5 + 0.5) * pu.bufferDim));
  var offset = (p.x * u32(pu.bufferDim) + p.y) * 2;
...
fn computeMain(
  ...
  for (var i = 0; i < 6; i++) {
    ...
    n += samp * step(1, pu.wrap + 1 - step(pu.gridRadius, amax3(vec3f(u))));
  }
