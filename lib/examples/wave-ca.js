...
export default (p) => {
  ...
  function centerFill() {
    ...
    for (let cell of cells) {
      setCell(data, cell, 16);
    }
  }

  function resetBoard() {
    p.dataBuffers[1].data.fill(0);
    settings.randomSeed && randomFill()
    settings.centerSeed && centerFill();
  ...
  return {
    ...
    controls: {
      ...
      centerSeed: false,
      pulseSettings: {
        pulseInterval: [60, 10, 360, 10],
        pulseMagnitudeFactor: [4, 0, 10, 1],
      },
      randomSettings: {
