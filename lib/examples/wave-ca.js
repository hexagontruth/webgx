...
export default (p) => {
  ...
  function blobFill() {
    const data = p.dataBuffers[0].data;
  ...
  function resetBoard() {
    p.dataBuffers[0].data.fill(0);
    settings.blobSeed && blobFill();
    p.dataBuffers[0].write();
  }
  ...
  return {
    ...
    uniforms,
    dataBuffers: [
      p.createDataBuffer(new Uint32Array(uniforms.bufferDim ** 2 * 2), p.DataBuffer.STORAGE_READ),
      p.createDataBuffer(uniforms.bufferDim ** 2 * 2, p.DataBuffer.STORAGE_WRITE),
    ],
    actions: {
      reset: () => {
        resetBoard();
      ...
      draw: async () => {
        ...
        p.copyBufferToBuffer(p.dataBuffers[1], p.dataBuffers[0]);
    ...
    pipelines: {
      compute: p.createComputePipeline(shader, { dataBuffers: [0, 1] }),
      main: p.createRenderPipeline(shader, { dataBuffers: [0, 1] }),
