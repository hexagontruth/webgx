...
export default (p) => {
  ...
  function flipCell(data, v) {
    v = math.fromHex(v, uniforms.bufferSize);
    const idx = v[0] * uniforms.bufferSize + v[1];
    data[idx] = 1 - data[idx];
  }
  ...
  function randomFill() {
    ...
    for (let i = 0; i < settings.blobCount; i++) {
      let v = Array(2).fill().map(() => floor(random() * uniforms.bufferSize));
      v = math.toHex(v, uniforms.bufferSize);
      v = math.wrapGrid(v, uniforms.gridSize);
  ...
  return {
    ...
    controls: {
      gridSize: [uniforms.gridSize, 5, floor(uniforms.bufferSize / 2 - 1), 1],
    ...
    dataBuffers: [
      ...
      p.createDataBuffer(uniforms.bufferSize ** 2, p.DataBuffer.STORAGE_READ),
      p.createDataBuffer(uniforms.bufferSize ** 2, p.DataBuffer.STORAGE_WRITE),
    ],
    actions: {
      reset: () => {
        resetBoard();
