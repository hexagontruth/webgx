...
export default (p) => {
  ...
  const settings = {
    ...
    blobCount: 64,
    blobSize: 8,
    bufferSize: uniforms.bufferDim ** 3,
  };

  const data = new Float32Array(settings.bufferSize);
  ...
  function symmetricFill() {
    ...
    for (let cell of cells) {
      flipCell(cell.map((e) => e + uniforms.bufferDim / 2));
    }
  }
  ...
  return {
    ...
    actions: {
      reset: () => {
        resetBoard();
        console.log(data.reduce((a, e) => a+e, 0));
      },
      draw: () => {
        p.renderWithEncoder((e) => {
          e.compute('compute', 32, 32, 32);
