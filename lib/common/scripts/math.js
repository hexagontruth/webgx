...
export function toHex(v, bufferDim, stride=1) {
  v = v.map((e) => e - bufferDim * stride / 2);
  v = [v[0] / stride, v[1] / stride, -v[0] / stride - v[1] / stride];
  return v;
}

export function fromHex(v, bufferDim, stride=1) {
  v = v.slice(0, 2).map((e) => (e + bufferDim / 2) * stride);
  return v;
}
